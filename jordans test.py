...
alpaca_api = alpaca.REST(
    key_id='PKU17ZF0Y6NFZUT3WWH6',
    secret_key='9RYJGplvbsIcYyaALMucCa3UWkudEzhoE8ckdu2o',
    ...
    if order_data.event == "new":
        ...
        for ordr in orders:
            if ordr["id"] == packaged_order["id"]:
                try:
                    alpaca_api.cancel_order(ordr["id"])
                    if ordr["side"] == "buy":
                        print("Increasing Bid")
                        # alpaca_api.submit_order(symbol=ordr["symbol"], qty=1, side="buy", type="limit", limit_price=ordr["limitPrice"] + .10, time_in_force="gtc")
                    if ordr["side"] == "sell":
                        print("Increasing Ask")
                        # alpaca_api.submit_order(symbol=ordr["symbol"], qty=1, side="sell", type="limit", limit_price=ordr["limitPrice"] - .10, time_in_force="gtc")
    ...
    if market_is_open and stock_data_exists:
        ...
        if spread > 1.25:
            filtered_positions = list(
                filter(lambda position: (position["symbol"] == expensive_class["sym"] and position["side"] == "short"), positions))
            filtered_orders = list(
                filter(lambda order: (order["symbol"] == expensive_class["sym"] and order["side"] == "sell"), orders))
            ...
            filtered_positions = list(
                filter(lambda position: (position["symbol"] == expensive_class["sym"] and position["side"] == "long"),
                       positions))
            filtered_orders = list(
                filter(lambda order: (order["symbol"] == expensive_class["sym"] and order["side"] == "buy"), orders))
            if len(filtered_positions) < 1 and len(filtered_orders) < 1:
